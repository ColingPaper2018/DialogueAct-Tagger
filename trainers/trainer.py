...
class Trainer:
    def __init__(
        self, config: Config, taxonomy: Taxonomy, corpora_list: List[Tuple[type, str]]
    ):
        self.config = config
        ...
        if len(corpora_list) == 0:
            logger.error(
                "There are no corpora loaded, and the classifier won't train. "
        ...
        for corpus in corpora_list:
            try:
                ...
                logger.error(
                    "DialogueActTrain error - The corpora list contains objects which are not corpora"
                )
                logger.error(
                    "Please ensure each of your corpora is a subclass of Corpus"
                )
                exit(1)
