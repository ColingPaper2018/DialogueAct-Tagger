...
class TransformerTrainer(Trainer):
    ...
    def train_transformer(self, train_set: List[Utterance], valid_set: List[Utterance], n_classes: int,
        ...
        if len(train_set) == 0 or n_classes < 2:
            logger.info(f"Skipping training for {model_name}; dataset length was {len(train_set)}, "
                        f"n_classes={n_classes}")
            self.save_checkpoint(model, model_name, 1.0)
            return model

        train_iter = BucketIterator(TransformerTagger.build_features(train_set, self.config),
        ...
        valid_iter = BucketIterator(TransformerTagger.build_features(valid_set, self.config),
                                    batch_size=self.config.batch_size,
                                    sort_key=lambda x: len(x.Text),
                                    device=self.config.device,
                                    train=True,
    ...
    def train(self, dump=True):
        ...
        if "dimension" in self.config.taxonomy.value.__annotations__.keys():
            ...
            dimension_labels = [
                ...
                for utt in dimension_train
            ]

            dimension_values = list(set([label for tagset in dimension_labels for label in tagset]))

            models['dimension'] = self.train_transformer(dimension_train, dimension_dev, len(dimension_values),
            ...
            for dimension_value in dimension_values:
                ...
                comm_labels = [
                    [tag for tag in utt.tags]
                    for utt in comm_train
                ]

                comm_values = list(set([label for tagset in comm_labels for label in tagset]))
                models[f'comm_{dimension_value}'] = self.train_transformer(comm_train, comm_dev, len(comm_values),
            ...
            models['comm_all'] = self.train_transformer(comm_train, comm_dev, len(comm_values), model_name='comm_all')
