...
class Switchboard(Corpus):
    ...
    def validate_corpus(self, folder):
        return (
            ...
            and os.path.exists(f"{folder}/sw00utt")
            ...
            if file_folder.startswith("sw"):  # dialog folder
                ...
                for filename in os.listdir(folder + "/" + file_folder):
                    if filename.startswith("sw"):  # dialog file
                        filelist[destination].append(
                            folder + "/" + file_folder + "/" + filename
                        )
    ...
    def parse_corpus(self, conversations):
        ...
        for destination in ["train", "test"]:
            for filename in conversations[destination].keys():
                ...
                segment = 0
                prev_tags = {
                    0: self.da_to_taxonomy("%", self.taxonomy, []),
                    1: self.da_to_taxonomy("%", self.taxonomy, []),
                }
                prev_texts = {0: "", 1: ""}
                for line in conversations[destination][filename]:
                    ...
                    sentence = re.sub(
                        r"([+/\}\[\]]|\{\w)", "", sentence
                    )  # this REGEX removes prosodic information and disfluencies
                    sentence = re.sub(
                        r"\W+", " ", sentence
                    )  # this REGEX removes non alphanumeric characters
                    sentence = " ".join(
                        sentence.split()
                    )  # this is just to make extra spaces collapse
                    tags = self.da_to_taxonomy(
                        sw_tag, self.taxonomy, prev_tags[speaker]
                    )
                    utterances[destination].append(
                        Utterance(
                            ...
                            context=[
                                Utterance(
                                    prev_texts[speaker], prev_tags[speaker], [], speaker
                                )
                            ],
                            speaker_id=speaker,
                        )
    ...
    @staticmethod
    def da_to_taxonomy(
        dialogue_act: str, taxonomy: Taxonomy, context: List[Tag]
    ) -> List[Tag]:
        if dialogue_act == "+":
            new_tag = context
        else:
            if taxonomy == Taxonomy.SWDA:
                mapping_dict = {
                    ...
                    "bh": [
                        SWDATag(comm_function=SWDAFunction.BackchannelInQuestionForm)
                    ],
                    "^q": [SWDATag(comm_function=SWDAFunction.Quotation)],
                    "bf": [SWDATag(comm_function=SWDAFunction.SummarizeReformulate)],
                    "na": [
                        SWDATag(comm_function=SWDAFunction.AffirmativeNonYesAnswers)
                    ],
                    "ny^e": [
                        SWDATag(comm_function=SWDAFunction.AffirmativeNonYesAnswers)
                    ],
                ...
                mapping_dict = {
                    ...
                    # "sv": [ISOTag(dimension=ISODimension.Task, comm_function=ISOTaskFunction.Statement)],
                    "qrr": [
                        ISOTag(
                            dimension=ISODimension.Task,
                            comm_function=ISOTaskFunction.ChoiceQ,
                        )
                    ],
                    "qw^d": [
                        ISOTag(
                    ...
                    "fc": [
                        ISOTag(
                            dimension=ISODimension.SocialObligation,
                            comm_function=ISOSocialFunction.Salutation,
                        )
                    ],
                    "br": [
                        ISOTag(
            ...
            if (
                new_tag == "%"
            ):  # mapping without rhetorical tags (see WS97 mapping guidelines for more details)
                new_tag = mapping_dict.get(
                    dialogue_act.split(",")[0]
                    .split(";")[0]
                    .split("^")[0]
                    .split("(")[0]
                    .replace("*", "")
